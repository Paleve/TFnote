...
class Block(collections.namedtuple('Block', ['scope', 'unit_fn', 'args'])):
    'A named tuple describiing a ResNet block.'
...
@slim.add_arg_scope
def stack_blocks_dense(net, blocks, outputs_collections):
    for block in blocks:
        with tf.variable_scope(block.scope, 'block', [net]) as sc:
            for i, unit in enumerate(block.args):
                with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
                    unit_depth, unit.depth_bottleneck, unit_stride = unit
                    net = block.unit_fn(net, depth=unit_depth,
                                        depth_bottleneck=unit_depth_bottleneck, stride=unit_stride)
...
def resnet_arg_scope(is_training=True, weight_decay=0.0001,
    ...
    with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(weight_decay),
                        ...
                        activation_fn=tf.nn.relu, normalizer_fn=slim.batch_norm_parames):
...
def bottleneck(inputs, depth, depth_bottleneck, stride,
    ...
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        residual = conv2d_same(residual, depth_bottleneck, 3, stride=1,
                               normalize_fn=None, activation_fn=None,
                               scope='conv2')
...
def resnet_v2_101(inputs, num_classes=None, global_pool=True,
                  reuse=True, scope='resnet_v2_101'):
    blocks = [
        Block('block1', bottleneck, [(256, 64, 1)] * 2 + [(256, 64, 2)]),
        Block('block2', bottleneck, [(512, 128, 1)] * 3 + [(512, 128, 2)]),
...
def resnet_v2_152(inputs, num_classes=None, global_pool=True,
                  reuse=True, scope='resnet_v2_152'):
    blocks = [
        Block('block1', bottleneck, [(256, 64, 1)] * 2 + [(256, 64, 2)]),
        Block('block2', bottleneck, [(512, 128, 1)] * 7 + [(256, 128, 2)]),
        Block('block3', bottleneck, [(1024, 256, 1)] * 35 + [(512, 256, 2)]),
        Block('block4', bottleneck, [(2048, 512, 1)] * 3)]

    return resnet_v2(inputs, blocks, num_classes, global_pool,
                     include_root_block=True, reuse=reuse, scop=scope)


def resnet_v2_200(inputs, num_classes=None, global_pool=True,
                  reuse=True, scope='resnet_v2_200'):
    blocks = [
        Block('block1', bottleneck, [(256, 64, 1)] * 2 + [(256, 64, 2)]),
        Block('block2', bottleneck, [(512, 128, 1)] * 23 + [(512, 128, 2)]),
        Block('block3', bottleneck, [(1024, 512, 1)] * 35 + [(1024, 512, 2)]),
        Block('block4', bottleneck, [(2048, 512, 1)] * 3)]
    return resnet_v2(inputs, blocks, num_classes, global_pool,
                     include_root_block=True, reuse=reuse, scope=scope)
